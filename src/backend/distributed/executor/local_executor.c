...
ExecuteLocalUtilityTaskList(List *localTaskList)
{
	...
	foreach_ptr(localTask, localTaskList)
	{
		...
		if (localTask->anchorShardId != INVALID_SHARD_ID)
		{
			 ...
			 * of the following commands withing the current transaction.
 ...
 * session. Given query would mostly be a utility command as the name of the
 * function implies. However, some utility commands can trigger udf calls to
 * finish their execution. See the comment in function.
 */
static void
